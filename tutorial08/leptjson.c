    ...
    switch (lhs->type) {
        ...
        case LEPT_OBJECT:
            ...
            for(i = 0; i < lhs->u.o.size; i++){
                const char* p = lhs->u.o.m[i].k;
                size_t len = lhs->u.o.m[i].klen;
                if(lept_find_object_index(rhs, p, len) == LEPT_KEY_NOT_EXIST)
                    return 0;
                if(lept_is_equal(lept_find_object_value(lhs, p, len), lept_find_object_value(rhs, p, len)) == 0)
                    return 0;
            }
            return 1;
        default:
...
void lept_erase_array_element(lept_value* v, size_t index, size_t count) {
    ...
    for(i = 0; i < count; i++){
        lept_move(&v->u.a.e[index + i], &v->u.a.e[index + i + count]);
    }
    v->u.a.size -= count;
}
